...
def get(token, config={}, toprint=False):
    ...
    if not ipas:
        # no longer doing ("'s","z")-- too simple
    ...
    toreturn = [(a, b, {'is_elision': c})
                for ((a, b), c) in zip(results, iselision)]
...
def syllabify_cmu(cmu_token):
    ...
    cmu_token = cmu_token.replace(' 2', '2').replace(
        ' 1', '1')  # fix prim/sec stress markings for syllabify
...
def syllabify_orth(token, num_sylls=None, func=syllabify_orth_with_nltk):
    ...
    if not key in ORTH_CACHE:
        ...
        # ,num_sylls=num_sylls) if (num_sylls and num_sylls>1) else func(tok)
        l = func(tok)
        if num_sylls:
...
def openmary(line):
    ...
    # print '>> openmary:',line
    # apostrophes seem to make openmary oversyllabify by a mile
    line = line.replace("'", "")
    line = unidecode(line)
    # print '>> openmary:',line
    # print
    ...
    line = line.replace(' ', '+')
    link = 'http://localhost:59125/process?INPUT_TEXT={0}&INPUT_TYPE=TEXT&OUTPUT_TYPE=ALLOPHONES&LOCALE=en_US'.format(
        line)
    ...
    for para in xml.find_all('p'):
        for phrase in para.find_all('phrase'):
            wordlist = [word for word in phrase.find_all(
                't') if len(list(word.find_all('syllable')))]
