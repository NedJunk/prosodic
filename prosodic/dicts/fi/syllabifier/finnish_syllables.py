...
# map between sequences of three and for vowels and their syllabifications [modelled after Karlsson 1985: (2b), but using Karlsson 1982 T-5/T-7 to deal with 'ieu', 'uoi']
vowel_seq_dict = {}
...
def apply_3c(word, boundary_list):
    ...
    if len(word) > seq_len:
        ...
        if word[-seq_len:] == sequence and word[-(seq_len+1)] != 't':
            ...
            # last entry is for word-final syllable boundary
            boundary_list[-3] = 1
...
def apply_t4(word, boundary_list):
    ...
    # check for rule application at syllable boundary (including word end); first possible boundary at index 3 (VVC-)
...
def separate_vowels(vowels, boundary_list, start):
    ...
    if v_len == 2 and not is_inseparable_vowels(vowels):
        ...
        if vowels in vowel_seq_dict:
            ...
            # ignore initial syllable separator and first vowel
            ...
            if boundary != -1:
                ...
                if boundary == 0:
                    ...
                    boundary = 2
                    # syllabify vowels following long vowel
                    separate_vowels(vowels[boundary:],
                                    boundary_list, start+boundary)

                else:

                    # syllabify vowels preceding long vowel
                    separate_vowels(vowels[:boundary], boundary_list, start)

                # split vowel from long vowel
                ...
                for i in range(len(vowels)-1):
                    ...
                    if not is_inseparable_vowels(vowels[i:i+2]):
                        ...
                        # insert boundary before the second vowel
...
def make_syllables(word):
    ...
    # syllable separator added to ensure that final vowel sequence is syllabified
    ...
    for i in range(len(word)):
        ...
        if is_vowel(word[i]):  # continuing or starting vowel sequence
            ...
            if v_seq_end - v_seq_start > 1:
                ...
                separate_vowels(word[v_seq_start:v_seq_end],
                                boundary_list, v_seq_start)

            # vowel sequence (if any) starts after current index
