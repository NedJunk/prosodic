...
class Parse(entity):
    ...
    def extend(self, slot):
        ...
        if len(self.positions) == 0:
            ...
            if lastPos.meterVal == 's':
                if len(lastPos.slots) < self.meter.maxS() and not slot.issplit:
                    # parse with extended final 's' position
                    sParse = copy(self)
                ...
                if len(lastPos.slots) < self.meter.maxW() and not slot.issplit:
                    # parse with extended final 'w' position
                    wParse = copy(self)
            ...
            for constraint in self.constraints:
                vScore = constraint.violationScore(
                    self.positions[-2], pos_i=pos_i, slot_i=self.numSlots-1, num_slots=self.totalSlots, all_positions=self.positions, parse=self)
                if vScore == "*":
        ...
        if self.numSlots == self.totalSlots:
            ...
            for parse in extendedParses:
                for constraint in self.constraints:
                    vScore = constraint.violationScore(parse.positions[-1], pos_i=len(
                        parse.positions)-1, slot_i=self.numSlots-1, num_slots=self.totalSlots, all_positions=parse.positions, parse=parse)
                    if vScore == "*":
    ...
    def canCompare(self, parse):
        ...
        isTrue = (self.numSlots == self.totalSlots) or (
            (self.positions[-1].meterVal == parse.positions[-1].meterVal) and (len(self.positions[-1].slots) == len(parse.positions[-1].slots)))
        if isTrue:
            pass
