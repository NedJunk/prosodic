...
class Meter:
    ...
    def genMeters():
        meterd = {}
        meterd['*StrongSyllableWeakPosition [Shakespeare]'] = Meter(
            ['strength.w=>-p/1'], (1, 2), False)
        meterd['*WeakSyllableStrongPosition'] = Meter(
            ['strength.s=>-u/1'], (1, 2), False)
        meterd['*StressedSyllableWeakPosition'] = Meter(
            ['stress.w=>-p/1'], (1, 2), False)
        meterd['*UnstressedSyllableStrongPosition [Hopkins]'] = Meter(
            ['stress.s=>-u/1'], (1, 2), False)
        return meterd
    ...
    def __str__(self):
        ...
        constraint_slices = [constraint_slices[k]
                             for k in sorted(constraint_slices)]
        constraints = '\n\t\t'.join(
            ' '.join(c.name_weight for c in slicex) for slicex in constraint_slices)

        x = '<<Meter\n\tID: {5}\n\tName: {0}\n\tConstraints: \n\t\t{1}\n\tMaxS, MaxW: {2}, {3}\n\tAllow heavy syllable split across two positions: {4}\n>>'.format(
            self.name, constraints, self.posLimit[0], self.posLimit[1], bool(self.splitheavies), self.id)
        return x
    ...
    def __init__(self, config):
        ...
        if not constraints:
            self.constraints.append(Constraint(
                id=0, name="foot-min", weight=1, meter=self))
            self.constraints.append(Constraint(
                id=1, name="strength.s=>p", weight=1, meter=self))
            self.constraints.append(Constraint(
                id=2, name="strength.w=>u", weight=1, meter=self))
            self.constraints.append(Constraint(
                id=3, name="stress.s=>p", weight=1, meter=self))
            self.constraints.append(Constraint(
                id=4, name="stress.w=>u", weight=1, meter=self))
            self.constraints.append(Constraint(
                id=5, name="weight.s=>p", weight=1, meter=self))
            self.constraints.append(Constraint(
                id=6, name="weight.w=>u", weight=1, meter=self))
            ...
            for i in range(len(constraints)):
                ...
                if "/" in c:
                    ...
                    sigmaVal = 10000
                self.constraints.append(Constraint(
                    id=i, name=cname, weight=cweight, meter=self, mu=muVal, sigma=sigmaVal))
        """
		else:
        ...
        for row in rows_wordmatrix:
            ...
            for word in row:
                ...
                for unit in word.children:  # units = syllables
                    ...
                    slot = Slot(id, unit, word.sylls_text[syllnum], wordpos, word, i_word=wordnum,
                                i_syll_in_word=syllnum, wordtoken=row_wordtokens[wordnum], meter=self)
            ...
            if not self.splitheavies:
                ...
                for slot in unitlist:
                    if bool(slot.feature('prom.weight')):
                        slot1 = Slot(
                            slot.i, slot.children[0], slot.token, slot.wordpos, slot.word)
                        slot2 = Slot(
                            slot.i, slot.children[0], slot.token, slot.wordpos, slot.word)
    ...
    def parse(self, wordlist, numSyll=0, numTopBounded=10):
        numTopBounded = self.prosodic_config.get(
            'num_bounded_parses_to_store', numTopBounded)
        ...
        for slots_i, slots in enumerate(slotMatrix):
            ...
            if maxsec and time.time()-clockstart > maxsec:
                print('!! Time limit ({0}s) elapsed in trying to parse line:'.format(
                    maxsec), ' '.join(wtok.token for wtok in wordlist))
                return [], []
    ...
    # onlyBounded=True, [option done through "report" now]
    def printParses(self, parselist, lim=False, reverse=True):
        ...
        for i, parse in enumerate(parseiter):
            ...
            o += '-'*20+'\n'
            o += "[parse #" + str(l_i[i]+1) + " of " + str(n) + "]: " + \
                str(parse.getErrorCount()) + " errors"
