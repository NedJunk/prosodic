...
class Phoneme(entity):
    ...
    def distance(self, other):
        lfs1 = [self.featspace] if not self.children else [
            c.featspace for c in self.children]
        lfs2 = [other.featspace] if not other.children else [
            c.featspace for c in other.children]
        dists = []
        for fs1 in lfs1:
            for fs2 in lfs2:
        ...
        if not 'cons' in feats1 and not 'cons' in feats2:
            ...
            for v1x in v1:
                for v2x in v2:
                    # print v1x,v2x
                    vdist = math.sqrt(
                        (formantd[v1x][0] - formantd[v2x][0])**2 + (formantd[v1x][1] - formantd[v2x][1])**2)
                    # print "ADDING",vdist
                    vdists += [vdist]
