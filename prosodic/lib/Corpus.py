...
class Corpus(entity):
    ...
    def stats_lines(self, meter=None, all_parses=False):
        ...
        ofn = os.path.join(self.dir_results, 'stats', 'corpora', self.name,
                           self.name+'.lines.'+('meter='+meter.id if meter else 'unknown')+'.csv')
    ...
    def stats_lines_ot(self, meter=None, all_parses=False):
        ...
        ofn = os.path.join(self.dir_results, 'stats', 'corpora', self.name,
                           self.name+'.lines_ot.'+('meter='+meter.id if meter else 'unknown')+'.csv')
        if not os.path.exists(os.path.split(ofn)[0]):
            os.makedirs(os.path.split(ofn)[0])
        for dx in writegengen(ofn, _writegen):
            yield dx
    ...
    def stats_positions(self, meter=None, all_parses=False):
        ...
        ofn = os.path.join(self.dir_results, 'stats', 'corpora',
                           self.name, self.name+'.positions.csv')
        if not os.path.exists(os.path.split(ofn)[0]):
            os.makedirs(os.path.split(ofn)[0])
        for dx in writegengen(ofn, _writegen):
            yield dx
