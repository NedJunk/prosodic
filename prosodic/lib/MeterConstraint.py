...
class MeterConstraint:
    ...
    def name_weight(self):
        weight = str(self.weight) if int(
            self.weight) != self.weight else str(int(self.weight))
    ...
    def violationScore(self, meterPos, pos_i=None, slot_i=None, num_slots=None, all_positions=None, parse=None):
        ...
        if self.constr != None:
            ...
            violation = self.__hardparse(meterPos, pos_i=pos_i, slot_i=slot_i,
                                         num_slots=num_slots, all_positions=all_positions, parse=parse)
    ...
    def __hardparse(self, meterPos, pos_i=None, slot_i=None, num_slots=None, all_positions=None, parse=None):
        ...
        if '.' in self.name:  # kiparsky self.names
            ...
            # -u or -p: eg, if s=>-u, then NOT EVEN ONE s can be u(nprom)
            if promSite_prom[0:1] == "-":
                promSite_isneg = True
                promSite_prom = promSite_prom[1:]				# u or p
            else:
                # u or p: eg, if s=>p, then AT LEAST ONE s must be p(rom)
            ...
            # string 2 boolean: p:True, u:False
            ...
            if promSite_isneg:
                ...
                for slot in meterPos.slots:
                    ...
                    pstress_thresh = self.meter.config.get(
                        'phrasal_stress_threshold', PSTRESS_THRESH_DEFAULT)
                    try:
                    ...
                    bool_prom_type = bool(
                        slot_prom) if promType != 'phrasal_stress' else slot_prom <= pstress_thresh
            ...
            if name == 'footmin-w-resolution':
                if a.word != b.word:
                    return 0  # only applies within word-boundaries
                firstsyll_islight = bool(
                    a.feature('prom.weight', True)) == False
                firstsyll_isstressed = bool(
                    a.feature('prom.stress', True)) == True
                if not (firstsyll_islight and firstsyll_isstressed):
                    return self.weight

            if name == 'footmin-f-resolution':
                if a.word == b.word:
                    return 0  # only applies to word-boundaries
                if meterPos.meterVal == 's':
                    return self.weight  # cannot apply to strong positions
            ...
            if "wordbound" in name:
                ...
                # in the firs slot's (start,end) wordpos : if (start==end) :  then poss. (ii) above
        ...
        if self.name == 'word-elision':
            words = set([slot.word for slot in meterPos.slots if hasattr(
                slot.word, 'is_elision') and slot.word.is_elision])
            sylls = []
            for slot in meterPos.slots:
                sylls += slot.children
        ...
        # is this the end?
        is_end = slot_i + \
            1 == num_slots and meterPos.slots == all_positions[-1].slots
        ...
        if is_end:
            final_meter_str = ''.join(
                [''.join(pos.meterVal for slot in pos.slots) for pos in all_positions])
            ...
            if self.name.startswith('headedness'):
				...
				"""
                quasi_feet = [''.join(x) for x in tools.slice(
                    [pos.meterVal for pos in all_positions], slice_length=2, runts=False)]
                headedness = 'rising' if quasi_feet.count(
                    'ws') >= quasi_feet.count('sw') else 'falling'
                # print final_meter_str
                # print quasi_feet
                # print headedness
                # print
            ...
            if self.name.startswith('number_feet'):
                ...
                strong_pos = [
                    pos for pos in all_positions if pos.meterVal == 's']
            ...
            if self.name.startswith('posthoc'):
                ...
                if self.name == 'posthoc-standardize-weakpos':
                    weak_pos = [
                        pos for pos in all_positions if pos.meterVal == 'w']
                    if len(weak_pos) < 2:
                        return 0
                    weak_pos_types = [
                        ''.join('w' for slot in pos.slots) for pos in weak_pos]
                    maxcount = max([weak_pos_types.count(wtype)
                                   for wtype in set(weak_pos_types)])
