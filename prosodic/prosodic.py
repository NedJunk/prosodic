...
dir_meters = config.get(
    'path_meters', os.path.join(dir_prosodic_home, 'meters'))
dir_results = config.get(
    'path_results', os.path.join(dir_prosodic_home, 'results'))
dir_tagged = config.get('path_tagged_samples', os.path.join(
    dir_prosodic_home, 'tagged_samples'))
dir_corpus = config.get(
    'path_corpora', os.path.join(dir_prosodic_home, 'corpora'))
dir_nlp_data = config.get('path_nlp_data', os.path.join(
    dir_prosodic_home, 'nlp_libraries'))

config['meters'] = loadMeters(dir_meters, config)
METER = config['meter'] = config['meters'][config['meter']
                                           ] if 'meter' in config and config['meter'] else None
...
# set defaults
languages = [lang for lang in os.listdir(dir_dicts) if os.path.isdir(
    os.path.join(dir_dicts, lang))] if dir_dicts else ['en', 'fi']
lang = config['lang']
...
def start_interactive_mode():
    ...
    try:
        ...
        if os.path.exists(arg):
            ...
            if arg2 == 'stanford_parser':
                ...
                if not os.path.exists(dir_nlp_data):
                    os.makedirs(dir_nlp_data)
    ...
    while(text != "/exit"):
        ...
        msg += "\t\t/exit\texit\n"
        # msg+="#######################################################################\n\n"
        msg += "\n>> ["+str(round((time.time() - timestart), 2)
                            )+"s] prosodic:"+lang+"$ "
        ...
        if text == "/exit":
            ...
            data_aggregator = DataAggregator(
                METER, data_path, lang, is_tab_formatted=tab_not_pipe)
            ...
            learner.train(step=step_size, epochs=max_epochs, tolerance=gradient_norm_tolerance,
                ...
                for words in obj.words(flattenList=False):
                    ...
                    for match in wordobj.search(qq):
                        ...
                        if "Word" in str(type(match)):
                            ...
                            matchstr = str(match)
                        wordobj.om(makeminlength(str(matchcount), int(
                            being.linelen/6))+"\t"+makeminlength(str(wordobj), int(being.linelen))+"\t"+matchstr)
        ...
        #	print obj.search(SearchTerm(text.replace('/query','').strip()))
                ...
                for filename in os.listdir(dir_corpus):
                    ...
                    if os.path.isdir(os.path.join(dir_corpus, filename)):
                        ...
                        for filename2 in glob.glob(os.path.join(os.path.join(dir_corpus, filename), "*.txt")):
                            files.append(filename2.replace(
                                dir_corpus, '').replace(filename+'/', ''))
                        print("\t\t"+" | ".join(files))
                        print()
                    else:
                        if filename[-4] == ".txt":
            ...
            if not key_meterscheme:
                key_meterscheme = 'Meter Scheme'

            assess(os.path.join(path, fn), key_meterscheme=key_meterscheme,
                   key_parse=key_parse, key_line=key_line, meter=METER)
