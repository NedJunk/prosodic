...
class DependencyTree(Tree):
    ...
    _contractables = ("m", "s", "ll", "d", "nt", "re", "ve",
                      "'m", "'s", "'ll", "'d", "n't", "'re", "'ve")
    _punctTags = ('.', ',', ':')
...
class DependencyTreeParser(ParserI):
    ...
    def _parse_trees_output(output_):
        ...
        for line in output_.splitlines(False):
            if line == '' and finished_tree:
                res.append(
                    iter([DependencyTree.fromstring('\n'.join(cur_lines))]))
                cur_lines = []
                finished_tree = False
            else:
    ...
    def _execute(self, cmd, input_, verbose=False):
        ...
        with tempfile.NamedTemporaryFile(mode='wb', delete=False) as input_file:
            ...
            stdout, stderr = java(cmd, classpath=(
                self._stanford_jar, self._model_jar, self._ejml_jar), stdout=PIPE, stderr=PIPE)
...
def setup_module(module):
    ...
    try:
        ...
        raise SkipTest(
            'doctests from nltk.parse.stanford are skipped because the stanford parser jar doesn\'t exist')
...
if __name__ == '__main__':
    ...
    os.environ['STANFORD_MODELS'] = 'Stanford Library/stanford-parser-full-%s/stanford-parser-%s-models.jar' % (
        DATE, MODELS_VERSION)
    os.environ['STANFORD_EJML'] = 'Stanford Library/stanford-parser-full-%s/ejml-%s.jar' % (
        DATE, EJML_VERSION)
    parser = DependencyTreeParser(
        model_path='Stanford Library/stanford-parser-full-%s/edu/stanford/nlp/models/lexparser/englishRNN.ser.gz' % DATE)
    ...
    lps = 0
    t_0 = time.time()
    lines = sum(1 for line in codecs.open(
        'Text Book/Tolkien/%s.txt' % basename, encoding='utf-8'))
    try:
        with codecs.open('Text Book/Tolkien/%s.txt' % basename, encoding='utf-8') as f:
            for line in f:
                ...
                etc_h = int(etc)
                etc_m = (etc-etc_h)*60
                print('Line %d/%d: %.1f lpm, %dh %.1fm left         \r' %
                      (i, lines, lpm, etc_h, etc_m), end=' ')
                sys.stdout.flush()
    except:
        print('Stopped while parsing line %d   ' % i)
    pkl.dump(tuples, open('Pickle Jar/%s.pkl' %
             basename, 'w'), protocol=pkl.HIGHEST_PROTOCOL)
